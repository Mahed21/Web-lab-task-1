function formValidation() {
  ...
  if (fullName.match(regaxName)?.join("") == fullName) {
    ...
    const regaxEmail = /[0-9]{10}_(cse|eee|llb|bba)@lus.ac.bd/g;
    if (inputEmail.match(regaxEmail)?.join("") == inputEmail) {
      // number validate
      ...
      if (number.match(regaxNumber)?.join("") == number) {
        ...
        alert(
          "number should be 11 digit and it should be bangladeshi operator"
        );
      }
    } else {
      alert("Email Should be leading University");
    }
  } else {
